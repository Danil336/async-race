...
import pagination, { carsCounter } from "./pagitarion";
import { startOneCar } from "./carsRace";

type CarInfoArr = {
  [key: string]: string;
};

const generatedCar = (name: string, color: string, id: number) => `
<div class="car_info">
  <div class="car_buttons">
    <button class="car_control_btn select_car_btn" car-id="${id}">Select</button>
    <button class="car_control_btn delete_car_btn" car-id="${id}">Remove</button>
  </div>
  <div class="car_title">${name}</div>
</div>
<div class="all_about_car">
<div class="car_command_buttons">
  <button class="command_btn command_btn_go active" car-id="${id}">A</button>
  <button class="command_btn command_btn_stop" car-id="${id}">B</button>
</div>
<div class="car_and_finish">
  <div class="car_image" data-name="${name}">
 ...
 130 22 193 51 l64 29 -19 23 c-65 82 -198 227 -209 227 -7 0 -15 -4 -19 -9z"/>
  ...
  <div class="car_finish"><img src="./assets/images/red-flag.svg" alt=""></div>
</div>
</div>
<div class="road">
`;

export const renderCars = async (pageNumber = 1) => {
  const garageArea = document.querySelector(".cars");

  const carsOnePage = await getCars([
    { key: "_page", value: pageNumber },
    { key: "_limit", value: 7 },
  ]);

  const carsItems: CarInfoArr[] = carsOnePage.items;

  carsItems.forEach((carInfo) => {
    const car = document.createElement("div");
    car.classList.add("car");
    car.setAttribute("car-id", carInfo.id);
    car.innerHTML = generatedCar(carInfo.name, carInfo.color, +carInfo.id);

    garageArea?.appendChild(car);
  });
  ...
  pagination(); // render cars after first page

  createHonderdCars();
};

export const necesseryCars = async (pageNumber: number) => {
  const garageArea = document.querySelector(".cars");
  ...
  carsItems.forEach((carInfo) => {
    ...
    car.classList.add("car");
    car.setAttribute("car-id", carInfo.id);
    car.innerHTML = generatedCar(carInfo.name, carInfo.color, +carInfo.id);   // render cars after first page

    garageArea?.appendChild(car);
  });

  const startBtn = document.querySelector(".all_cars_race_btn") as HTMLButtonElement;

  startBtn.classList.remove("control_unactive");
...
const createOneCar = async () => {
  ...
  createBtn?.addEventListener("click", async function listen() {
    if(garageArea!.children.length <= 6) {
      const newCarName = createInput.value;
      const newCarColor = createColor.value;
      ...
      const LastAddedCar = allCars.items[allCars.items.length - 1];
  
      const newOneCar = document.createElement("div");
      newOneCar.classList.add("car");
      newOneCar.setAttribute("car-id", LastAddedCar.id);
      newOneCar.innerHTML = generatedCar(
        newCarName,
        newCarColor,
        LastAddedCar.id
      );
  
      garageArea?.appendChild(newOneCar);
...
const deleteOneCar = async () => {
  const allRemoveBtns = document.querySelectorAll(".delete_car_btn");

  allRemoveBtns.forEach((removeBtn) => {
    removeBtn.addEventListener("click", async () => {
      const thisCar = removeBtn.parentElement?.parentElement?.parentElement;
      const carId = Number(
        (removeBtn as HTMLDivElement).getAttribute("car-id")
      );
      await deleteCar(carId);
      thisCar?.parentElement?.removeChild(thisCar);
...
const changeOneCar = async () => {
  const updateInput = document.querySelector(".update_input") as HTMLInputElement;
  const updateColor = document.querySelector(".color_update_input") as HTMLInputElement;
  const updateBtn = document.querySelector(".update_button");
  const allSelectBtns = document.querySelectorAll(".select_car_btn");

  const selectedCars: number[] = [];

  allSelectBtns.forEach((selectBtn) => {
    selectBtn.addEventListener("click", async function selectListener() {

      const thisCar = selectBtn.parentElement?.parentElement?.parentElement;
      const carId = Number((selectBtn as HTMLDivElement).getAttribute("car-id"));
      const carCurrentName = (thisCar!.children[0].children[1] as HTMLDivElement).innerText;
      const carCurrentColor = thisCar!.children[1].children[1].children[0].children[0].children[1].getAttribute("fill");

      updateInput.value = carCurrentName;
      updateColor.value = carCurrentColor!;

      selectedCars.push(carId);

      updateBtn?.addEventListener("click", async function updateListener() {
        if(carId === selectedCars[selectedCars.length - 1]) {

          const newCarName = updateInput.value;
          const newCarColor = updateColor.value;
          ...
          thisCar!.innerHTML = generatedCar(newCarName, newCarColor, carId);
...
const createHonderdCars = () => {
  const createBtn = document.querySelector(".create_btn");
  const randomCarsArr = ["permeate", "embezz", "suscept", "misanth", "dissip", "obscure", "diligent", "cursory", "laudable", "iridesc", "lumber", "reminisce", "conflagration", "flora", "paragon", "gusto", "prologue", "divergent", "frenzy", "provocative", "paucity", "saunter", "elixir", "benevolent", "divert", "rejoice", "embellish", "flounder", "entice", "potential", "renowned", "pensive", "revelation", "bronze", "diffuse", "lucid", "glean", "lurk", "dally", "cower", "allay", "unfathomable", "quandary", "pique", "truculent", "glossy", "dubious", "gust", "pensive", "dilate", "hindrance", "laud", "exemplary", "flourish", "endear", "reticent", "dilute", "renowned", "ebullient", "gorgeous", "quaver", "pestilent", "flour", "gouge", "dormant", "gargantuan", "lurid", "quaver", "dour", "flourish", "flout", "diligent", "gusto", "hinder", "embellish", "exasperate", "dally", "dissipate", "lumber", "flounder", "laudable", "diffuse", "divergent", "misanthrope", "benevolent", "divert", "dormant", "embezzlement", "dilute", "permeate", "embezzlement", "susceptible", "misanthrope", "dissipate", "obscure", "diligent", "cursory", "laudable", "iridescent", "lumber"];

  createBtn?.addEventListener("click", async function createListener() {
    createBtn.removeEventListener("click", createListener);
    createBtn.classList.add("create_unactive");

    for(let i = 0; i < randomCarsArr.length; i++) {
      const carColor = getRandomHexColor();
      const carName = randomCarsArr[i];
      await createCar({
        name: carName,
        color: carColor,
      });
